...
# levs_keep <-  c("cm-cf", "lm-hf", "hm-hf", "lm-lf", "hm-lf", "mm-mf")
levs_keep <-  c("cm-cf", "lm-hf", "hm-hf", "lm-lf", "hm-lf")

cl_small <- 
  cl_robust_all %>% 
  filter(cohort %in% seq(1950, 2000, 5)) %>%
  filter(age %in% seq(15, 100, 10)) %>% 
  mutate(
    ...
    , variant = paste0(variant_mort, "-", variant_fert)
  ) %>% 
  filter( variant %in% levs_keep) %>% 
