...
births_medium_B <- readxl::read_xlsx(
  path= "../../Data/wpp_data/WPP2019_FERT_F01_BIRTHS_BOTH_SEXES.xlsx"
  , sheet = "MEDIUM VARIANT"
  , skip = 16
)

births_low_B <- readxl::read_xlsx(
  path= "../../Data/wpp_data/WPP2019_FERT_F01_BIRTHS_BOTH_SEXES.xlsx"
  , sheet = "LOW VARIANT"
  , skip = 16
)

births_high_B <- readxl::read_xlsx(
  path= "../../Data/wpp_data/WPP2019_FERT_F01_BIRTHS_BOTH_SEXES.xlsx"
  , sheet = "HIGH VARIANT"
  , skip = 16
)
...
# Predicted values for 2020-2100
# sex_ratio_at_birth_medium <- readxl::read_xlsx(
#   path= "../../Data/wpp_data/WPP2019_FERT_F02_SEX_RATIO_AT_BIRTH.xlsx"
#   , sheet = "MEDIUM VARIANT"
#   , skip = 16
# )

sex_ratio_at_birth_low <- 
  get_wpp_period_by_variant(fertility_variant = "low") %>% 
  select(country = region, year = Time, value = SRB) %>% 
  # Fix UN overlapping periods
  mutate(year = change_period_labels(year)) %>% 
  arrange(country, year)

sex_ratio_at_birth_medium <- 
  get_wpp_period_by_variant(fertility_variant = "mediume") %>% 
  select(country = region, year = Time, value = SRB) %>% 
  # Fix UN overlapping periods
  mutate(year = change_period_labels(year)) %>% 
  arrange(country, year)

sex_ratio_at_birth_high <- 
  get_wpp_period_by_variant(fertility_variant = "high") %>% 
  select(country = region, year = Time, value = SRB) %>% 
  # Fix UN overlapping periods
  mutate(year = change_period_labels(year)) %>% 
  arrange(country, year)
