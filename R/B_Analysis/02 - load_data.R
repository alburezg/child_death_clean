...
print(paste("NOTE!"))
print(paste("using fertility variant -", fertility_variant))

# csv version. including with different indicators for the given scenario
wpp_period_variant <- get_wpp_period_by_variant(fertility_variant)
...
fert_wpp_per <- wpp_period_variant %>% 
  select(
    region
    , period = Time
    , TFR
...
ASFRC <- read.csv(
  file = paste0("../../Data/derived/","ASFRC_",fertility_variant,".csv")
  , stringsAsFactors = F
  )

# Mean age at childbirth (MAC)

mac_wpp_per <- wpp_period_variant %>% 
  select(
    region
    , period = Time
    , value = MAC
...
all_births <- read.csv(
  file = paste0("../../Data/derived/","wpp_all_births_1_1_",fertility_variant,".csv")
  , stringsAsFactors = F
)
...
world_pop <- 
  ...
  mutate(Variant = tolower(Variant)) %>% 
...
# world_pop_age <- get_world_pop_age(fertility_variant = "low")
# world_pop_age <- get_world_pop_age(fertility_variant = "medium")
