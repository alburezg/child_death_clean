...
ggsave(paste0("../pres/fig1.pdf"), p_ex_ctfr, width = width, height = height, units = "cm")
...
f1 <- function(co) {
    ...
    geom_line(
      ...
      , linetype = 'dashed'
      , colour = "black"
      , data = country_lines %>% filter(cohort %in% c(1950:co))
      , show.legend = F
      , size = 0.7
    ) +
    # COuntry names
    ...
    geom_point(
      ...
      , size = 5
      , data = points %>% filter(cohort %in% 1950:co)
    ) +
    scale_x_continuous(
      "Cohort life expectancy at birth (years)"
    ) +
    ...
    scale_color_discrete("", br = regions_long, labels = regions_short) +
    scale_shape_discrete("", br = regions_long, labels = regions_short) +
    coord_cartesian(xlim = c(40, 87), ylim = c(1.6, 6.5)) +
    # scale_size_continuous("", range = c(1, 3), guide = F) +
    ...
    ggtitle(paste0("Birth cohorts: 1950-", co))
  
}

pdf("../pres/fig1_trans.pdf", width = 10, height = 7)
lapply(1950:1999, f1)
dev.off()
